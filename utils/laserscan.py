...
class LaserScan:
    ...
    EXTENSIONS_SCAN = ['.bin', 'hdf5']

    def __init__(self, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, noise_labels=None, pred_noise=None,
                 show_clear=False, show_denoised=False):
...
class SemLaserScan(LaserScan):
    ...
    def __init__(self, nclasses, sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0,
        ...
        super(SemLaserScan, self).__init__(project, H, W, fov_up, fov_down, noise_labels, pred_noise, show_clear,
                                           show_denoised)
        self.reset()
        ...
        if self.show_clear:
            self.sem_color_clearlut = self.sem_color_lut.copy()
            self.sem_color_clearlut[self.noise_labels] = 0
    ...
    def reset(self):
        ...
        if self.show_clear:
            self.proj_clear_label = np.zeros((self.proj_H, self.proj_W), dtype=np.int32)
            self.proj_clear_color = np.zeros((self.proj_H, self.proj_W, 3), dtype=np.float)

        if self.show_denoised:
            self.proj_denoised_label = np.zeros((self.proj_H, self.proj_W), dtype=np.int32)
            self.proj_denoised_color = np.zeros((self.proj_H, self.proj_W, 3), dtype=np.float)
    ...
    def open_prelabel(self, filename):
        """ Open raw scan and fill in attributes
        """
        # check filename is string
        if not isinstance(filename, str):
    ...
    def colorize(self):
        ...
        if self.show_clear:
            self.sem_label_clearcolor = self.sem_color_clearlut[self.sem_label]
        if self.show_denoised:
            self.sem_label_denoisedcolor = self.sem_color_lut[self.sem_prelabel_bin]
    ...
    def do_label_projection(self):
        ...
        if self.show_clear:
            self.proj_clear_label[mask] = self.sem_label[self.proj_idx[mask]]
            self.proj_clear_color[mask] = self.sem_color_clearlut[self.sem_label[self.proj_idx[mask]]]

        if self.show_denoised:
            self.proj_denoised_label[mask] = self.sem_prelabel_bin[self.proj_idx[mask]]
            self.proj_denoised_color[mask] = self.sem_color_lut[self.sem_prelabel_bin[self.proj_idx[mask]]]
