...
from model import Var_Nine as Net
import numpy as np
...
def get_data_loaders(data_dir, batch_size=None, num_workers=None, k_dis=False):
    ...
    test_loader = DataLoader(DENSE(root=data_dir, split='joke', transform=transforms, k_dis=k_dis),
...
def run(args):
    ...
    for iter, data in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            ...
            # flops, params = profile(model, inputs=([data[0][0][0].unsqueeze(1).cuda(), data[0][1][0].unsqueeze(1).cuda(), data[0][2][0].unsqueeze(1)],))
        ...
        # print(TP / (TP + FN))
        # dir_file = os.path.join(args.output_dir, file_id[0].split('/')[0])
        # if not os.path.exists(dir_file):
        #     os.makedirs(dir_file)
        # result_label_file = os.path.join(args.output_dir, file_id[0].split('/')[0], file_id[0].split('/')[1] + '_pred.label')
        # pred_output.astype('uint32').tofile(result_label_file)
        generate_pcbin = False
        if generate_pcbin:
            pcbin = os.path.join('result', 'denoised_snow_sim_8_2c_110', ''.join(file_id) + '.bin')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str,
                        # default='checkpoints/wads_3c/wads_3c_epoch287_mIoU=89.5.pth',
                        default='checkpoints/dense_3c/dense_3c_epoch47_mIoU=95.6.pth',
                        help='path to latest checkpoint (default: none)')
    parser.add_argument('--val-batch-size', type=int, default=1,
                        help='input batch size for validation to visualize result, Must Be 1')
    parser.add_argument('--num-workers', type=int, default=0,
                        help='number of workers')
    parser.add_argument('--output-dir', default='result/dense_2c_94.5',
